//! v0.24: AST 适配层
//!
//! 将旧 AST (ast.rs) 转换为新 AST (ast_v2.rs)
//! 支持渐进式迁移
...
impl AstAdapter {
    ...
    pub fn convert_stmt(&mut self, stmt: &Stmt) -> NodeId {
        ...
        let kind = match stmt {
            ...
            Stmt::Match {
                ...
                arms: arms
                    .iter()
                    .map(|(p, s)| (self.convert_pattern(p), self.convert_stmts(s)))
                    .collect(),
            },
    ...
    pub fn convert_expr(&mut self, expr: &Expr) -> NodeId {
        ...
        let kind = match expr {
            ...
            Expr::Match {
                ...
                arms: arms
                    .iter()
                    .map(|(p, e)| (self.convert_pattern(p), self.convert_expr(e)))
                    .collect(),
            },
    ...
    /// 转换 ObserveConfig
    fn convert_observe_config(&mut self, config: &crate::ast::ObserveConfig) -> crate::ast_v2::ObserveConfig {
        match config {
            crate::ast::ObserveConfig::Trace => crate::ast_v2::ObserveConfig::Trace,
            crate::ast::ObserveConfig::Metrics => crate::ast_v2::ObserveConfig::Metrics,
            crate::ast::ObserveConfig::Otel { endpoint } => {
                crate::ast_v2::ObserveConfig::Otel {
    ...
    /// 转换 FnDef
    fn convert_fn_def(&mut self, fndef: &crate::ast::FnDef) -> crate::ast_v2::FnDef {
        crate::ast_v2::FnDef {
    ...
    /// 转换 TraitMethod
    fn convert_trait_method(&mut self, method: &crate::ast::TraitMethod) -> crate::ast_v2::TraitMethod {
        crate::ast_v2::TraitMethod {
    ...
    /// 转换 Pattern
    fn convert_pattern(&mut self, pattern: &crate::ast::Pattern) -> crate::ast_v2::Pattern {
        match pattern {
            crate::ast::Pattern::Wildcard => crate::ast_v2::Pattern::Wildcard,
            crate::ast::Pattern::Literal(lit) => crate::ast_v2::Pattern::Literal(lit.clone()),
            crate::ast::Pattern::Variable(name) => crate::ast_v2::Pattern::Variable(name.clone()),
            crate::ast::Pattern::List { prefix, rest } => crate::ast_v2::Pattern::List {
                prefix: prefix.iter().map(|p| self.convert_pattern(p)).collect(),
                rest: rest.clone(),
            },
            crate::ast::Pattern::Dict(entries) => crate::ast_v2::Pattern::Dict(
                entries.iter().map(|(k, p)| (k.clone(), self.convert_pattern(p))).collect(),
            ),
            crate::ast::Pattern::Guard { pattern, condition } => crate::ast_v2::Pattern::Guard {
