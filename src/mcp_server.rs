...
pub fn start(tools: ToolRegistry, interpreter: Arc<Mutex<Interpreter>>) -> io::Result<()> {
    ...
    loop {
        ...
        // dispatch
        let response = dispatch(&req, &tools, &interpreter);

        // 写响应 (notifications 不响应)
...
fn dispatch(req: &JsonValue, tools: &ToolRegistry, interp: &Arc<Mutex<Interpreter>>) -> Option<JsonValue> {
        ...
        _ => {
            ...
            if !is_notification {
                ...
                Some(JsonValue::Object({
                    ...
                    m.insert("error".to_string(), JsonValue::Object({
                        let mut e = std::collections::BTreeMap::new();
                        e.insert("code".to_string(), JsonValue::Number(-32601.0));
                        e.insert("message".to_string(), JsonValue::String_(format!("Method not found: {}", method)));
                        e
                    }));
                    m
                }))
...
fn handle_initialize(_req: &JsonValue) -> JsonValue {
    ...
    result.insert("protocolVersion".to_string(), JsonValue::String_("2024-11-05".to_string()));
    ...
    result.insert("serverInfo".to_string(), JsonValue::Object({
        ...
        s.insert("version".to_string(), JsonValue::String_("0.04".to_string()));
        s
    }));
...
fn handle_tools_call(req: &JsonValue, tools: &ToolRegistry, interp: &Arc<Mutex<Interpreter>>) -> JsonValue {
    ...
    let args_json = params.and_then(|p| p.get("arguments")).cloned().unwrap_or(JsonValue::Object(std::collections::BTreeMap::new()));
    ...
    // 调 handler 闭包
    let result = {
        let mut interp = interp.lock().unwrap();
    ...
    let content_item = match result_json {
        ...
        other => {
            ...
            let mut c = std::collections::BTreeMap::new();
            c.insert("type".to_string(), JsonValue::String_("text".to_string()));
            c.insert("text".to_string(), JsonValue::String_(json_to_string(&other)));
            JsonValue::Object(c)
        }
    };
...
fn mcp_error(code: i64, message: &str) -> JsonValue {
    ...
    m.insert("message".to_string(), JsonValue::String_(message.to_string()));
    JsonValue::Object(m)
}
