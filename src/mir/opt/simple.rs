...
pub(super) fn const_propagate(ssa: &mut MirSsaFunction) {
...
pub(super) fn dead_code_elim(ssa: &mut MirSsaFunction) {
    for block in &mut ssa.blocks {
...
pub(super) fn global_value_numbering(ssa: &mut MirSsaFunction) {
    for block in &mut ssa.blocks {
