...
impl Parser {
    ...
    fn unary(&mut self) -> Expr {
        if self.match_token(&[TokenType::Minus]) {
            let op = self.previous_op();
            let right = self.unary();
            Expr::Binary { left: Box::new(Expr::Literal(Literal::Number(0.0))), op, right: Box::new(right) }
    ...
    fn pattern(&mut self) -> Option<Pattern> {
        ...
        if self.match_token(&[TokenType::True]) { Some(Pattern::Literal(Literal::Bool(true))) }
        else if self.match_token(&[TokenType::False]) { Some(Pattern::Literal(Literal::Bool(false))) }
        else if self.match_token(&[TokenType::Nil]) { Some(Pattern::Literal(Literal::Nil)) }
    ...
    fn primary(&mut self) -> Expr {
        if self.match_token(&[TokenType::True]) { Expr::Literal(Literal::Bool(true)) }
        else if self.match_token(&[TokenType::False]) { Expr::Literal(Literal::Bool(false)) }
        else if self.match_token(&[TokenType::Nil]) { Expr::Literal(Literal::Nil) }
            ...
            if !self.check(&TokenType::RBracket) {
                ...
                while self.match_token(&[TokenType::Comma]) { items.push(self.expression()); }
            }
            ...
            if !self.check(&TokenType::RBrace) {
                ...
                while self.match_token(&[TokenType::Comma]) { entries.push(self.dict_entry()); }
            }
            self.consume(&TokenType::RBrace, "Expected '}' after dict");
            Expr::Literal(Literal::Dict(entries))
        }
        else if let Some(Token { token_type: TokenType::Identifier(name), .. }) = self.peek().cloned() {
            self.advance();
            Expr::Variable(name)
        ...
        while let Some(ch) = chars.next() {
            if ch == '{' {
                if chars.peek() == Some(&'{') {
                    ...
                    if !current.is_empty() {
                        parts.push(Expr::Literal(Literal::String(current.clone())));
                        current.clear();
                    }
    ...
    fn arguments(&mut self) -> Vec<Expr> {
        ...
        while self.match_token(&[TokenType::Comma]) { args.push(self.expression()); }
        args
    }
