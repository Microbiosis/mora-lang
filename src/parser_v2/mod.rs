...
#[cfg(test)]
mod tests {
    //! ParserV2 顶层 API 与共享辅助白盒测试。
    //!
    //! 覆盖：
    //! - ParserV2::new / parse / arena / arena_mut / into_arena 生命周期
    //! - match_binary_op 识别所有支持的算符
    //! - previous_binary_op 反向映射正确
    //! - EOF / token 流边界
    //!
    //! 注意:表达式 / 语句 路径分别在 expressions / statements 单测覆盖,本模块只测
    //! 顶层 API 与共享辅助。
    use super::*;
    ...
    #[test]
    fn parse_returns_let_at_top_level() {
        let tokens = Lexer::new("let x = 1").scan_tokens();
        let mut parser = ParserV2::new(tokens);
        let stmts = parser.parse();
        assert_eq!(stmts.len(), 1);
        let arena = parser.into_arena();
        let s = arena.get_stmt(stmts[0]).unwrap();
        assert!(matches!(s.kind, StmtKind::Let { .. }));
    }

    #[test]
    fn parse_skips_leading_and_trailing_newlines() {
    ...
    #[test]
    fn parse_multiple_statements_in_sequence() {
    ...
    #[test]
    fn match_binary_op_matches_plus_token() {
        ...
        let mut parser = ParserV2::new(tokens);
        assert!(parser.match_binary_op());
    }

    #[test]
    fn match_binary_op_rejects_identifier() {
        let tokens = vec![Token {
    ...
    #[test]
    fn match_binary_op_matches_all_eleven_supported_ops() {
        // 11 个: + - * / % == != > < >= <=
        for tt in [
            TokenType::Plus,
            TokenType::Minus,
            ...
            let mut parser = ParserV2::new(tokens);
            assert!(parser.match_binary_op(), "expected to match {tt:?}");
        }
    }
}
